...
exports.getGitDiff = self.getGitDiff = function (cwd,base,head,includePatch){
	...
	let raw = self.execSync(("git diff --raw --numstat " + base + ".." + head),cwd).toString();
	let lines = raw.split('\n');
	...
	for (let i = 0, items = iter$(lines), len_ = items.length, entry; i < len_; i++) {
		...
		if (includePatch) {
			...
			if (mode == 'A') {
				...
				let patch = self.execSync(("git diff " + base + ".." + head + " " + file),cwd).toString();
				node.patch = patch;
			};
		};
...
Git.prototype.execAsync = function (cmd){
	...
	return new Promise(function(resolve,reject) {
		return cp.exec('git ' + cmd,{cwd: self.cwd(),env: process.env,maxBuffer: 1024 * 500},function(err,stdout,stderr) {
...
GitRepo.prototype.grep = async function (text){
	...
	let rootSha = await this.execAsync('merge-base HEAD @{u}');
	
	this.log("grep",text,rootSha);
	let lines = text.split('\n');
	let cmd = "grep --files-with-matches --all-match -n -F $'" + text.replace(/\'/g,'\\\'') + "'";
	let matches = [];
	let res = await this.execAsync(cmd);
	this.log("grep",cmd,res);
	
	if (res) {
		for (let i = 0, items = iter$(res.split("\n")), len = items.length, file; i < len; i++) {
			file = items[i];
			this.log(("git cat-file -p " + rootSha + ":" + file));
			try {
				let body = await this.execAsync(("cat-file -p " + rootSha + ":" + file));
				let start = 0;
				let idx;
				...
				while ((idx = body.indexOf(text,start)) >= 0){
					...
					// include full lines?
					let url = ("https://github.com/" + this.gitRepoRef() + "/blob/" + rootSha + "/" + file + "#L" + (match.line) + "-L" + (match.line + len$(lines)));
