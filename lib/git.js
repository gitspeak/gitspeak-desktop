...
self.valid = function (str){
	return str.match(/^[a-z0-9]+$/);
};

self.tryFetch = function (cwd,sha,type){
...
exports.getGitBlob = self.getGitBlob = function (cwd,sha,returnAnyway){
	...
	try {
		...
		if (returnAnyway) {
			...
			return self.tryFetch(cwd,sha,'blob');
		};
	};
};

exports.getGitTree = self.getGitTree = function (cwd,sha,returnAnyway){
	var ary;
	if(returnAnyway === undefined) returnAnyway = false;
	if (!self.valid(sha)) {
		return null;
	...
	try {
		...
		if (returnAnyway) {
			...
			return self.tryFetch(cwd,sha,'tree');
		};
	};
};
