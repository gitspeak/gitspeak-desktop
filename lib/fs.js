...
FileSystem.prototype.get = function (src,pars){
	var git_;
	...
	if (stat.isFile()) {
		...
		if (node.status) { this.loadNodeBody(node) };
	};
...
FileSystem.prototype.loadNodeBody = function (node,emitRead){
	...
	if (node.type == 'file') {
		let prev = node.body;
		if (!this.isBinary(node.path) && node.size < 200000) {
			node.body = this._contents[path] = this.extfs().readFileSync(this.absPath(path),'utf-8') || "";
			node.lazy = false;
			node.flags = node.flags & (~FLAGS.LAZY);
			// if emitRead
			// 	console.log "emit nodeBody",path,node:body
			if (node.body != prev && emitRead) {
				return this.emit('read',path,node.body);
			};
		};
	};
};
...
FileSystem.prototype.read = function (path){
	...
	let node = this.get(path,{recursive: true});
...
FileSystem.prototype.watchDir = function (node){
	...
	for (let i = 0, items = iter$(this.extfs().readdirSync(abs)), len = items.length; i < len; i++) {
		this.get(path.join(abs,items[i]));
	};
	return this;
};
...
FileSystem.prototype.mv = function (src,dest){
	...
	this.log("mv",src,dest);
	// console.log "move command",src,dest
	if (src && dest && !this.get(dest)) {
		return this.extfs().renameSync(src,dest);
	};
};
