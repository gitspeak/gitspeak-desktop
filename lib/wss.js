...
SocketClient.prototype.onmessage = async function (msg){
	this.log("wss.onmessage",msg[0],msg[1]);
	let callback;
	if ((typeof msg[0]=='number'||msg[0] instanceof Number)) {
		callback = msg.shift();
	};
	
	if (this.widget() && (this.widget()[msg[0]] instanceof Function)) {
		let res = await this.widget()[msg[0]].apply(this.widget(),msg[1]);
		if (callback) { this.send([-callback,res]) };
	};
	return this;
};
