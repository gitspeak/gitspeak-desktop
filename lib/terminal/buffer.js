...
exports.getFill = self.getFill = function (width,char$){
	return new Array(width + 1).join(char$);
};

exports.getWhitespace = self.getWhitespace = function (width){
	return self.getFill(width," ");
};

self.createColor = function (source,color){
	if (source == 'rgb') {
...
var TextStyling = exports.TextStyling = {
	...
	fromAttributes: function(attrs) {
		...
		return [
			self.createColor(attrs.foregroundSource,attrs.foreground),
			self.createColor(attrs.backgroundSource,attrs.background),
			bitfield
		];
	}
};
...
Line.prototype.erase = function (column,width,styling){
	...
	if (after) {
		var space = this.getWhitespace(width);
...
Line.prototype.replace = function (column,text,styling){
	...
	if (currentWidth < column) {
		this._text += this.getWhitespace(column - currentWidth);
	};
